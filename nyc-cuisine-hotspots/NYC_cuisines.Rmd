...
A critical detail for spatial clustering is that raw latitude/longitude coordinates (WGS84) distort distances. At NYC's latitude (~40.7°N):
- 1° latitude ≈ 111 km (constant everywhere)
- 1° longitude ≈ 85 km (shrinks toward poles: 111 × cos(40.7°))

If we ran DBSCAN on raw lat/long, the `eps` parameter would have different meanings for north-south vs. east-west distances — creating elliptical neighborhoods instead of circular ones.

To fix this, we **project** coordinates to EPSG:2263 (New York State Plane, Long Island), a local coordinate system designed for NYC that uses feet as its unit. After converting to meters, our `eps` parameter represents true distance: `eps = 500` means 500 meters in all directions.

There are two functions: one for basic DBSCAN, one that runs 'auto' to adjust for varying presences of each cuisine across the city. This is done by modifying `minPts`, the number of points needed to form a cluster, i.e., the number of restaurants needed to be considered a hotspot of that cuisine. 

```{r clustering}
# Define the projected CRS for NYC (State Plane Long Island, feet)
# We'll use EPSG:2263 and convert feet to meters for DBSCAN
NYC_CRS <- 2263  # New York State Plane (Long Island), units = US survey feet
FEET_TO_METERS <- 0.3048006096  # conversion factor

run_dbscan_for_cuisine <- function(data, cuisine, eps = 500, minPts = 5) {
  # NOTE: eps is now in METERS, not degrees!
  # eps = 500 means "restaurants within 500 meters are neighbors"
 ...
 # 1. Filter for the chosen cuisine and remove null island coords
  ...
  # 2. Convert to sf object with WGS84 (lat/long) coordinates
  cuisine_sf <- st_as_sf(
    cuisine_data,
    coords = c("longitude", "latitude"),
    crs = 4326,        # WGS84 = raw lat/long
    remove = FALSE     # keep original lat/long columns for later mapping
  )
  
  # 3. Project to NYC State Plane (EPSG:2263) for accurate distance calculations
  #    This CRS uses feet, so we extract coordinates and convert to meters
  cuisine_projected <- st_transform(cuisine_sf, crs = NYC_CRS)
  
  # 4. Extract projected coordinates (in feet) and convert to meters
  proj_coords <- st_coordinates(cuisine_projected)  # returns X, Y in feet

  proj_coords_meters <- proj_coords * FEET_TO_METERS  # now in meters
  
  # 5. Run DBSCAN on projected coordinates
  #    Now eps = 500 truly means 500 meters in ALL directions (no distortion)
  db_result <- dbscan::dbscan(proj_coords_meters, eps = eps, minPts = minPts)
  
  # 6. Add cluster assignments back to original data (with lat/long intact)
...
run_dbscan_for_cuisine_auto <- function(data, cuisine, eps = 750, min_floor = 3, prop_factor = 0.02) {
  # NOTE: eps is now in METERS, not degrees!
 ...
 # 1. Filter for the chosen cuisine and remove null island coords
  ...
  # 3. Convert to sf and project to NYC State Plane for accurate distances
  cuisine_sf <- st_as_sf(
    cuisine_data,
    coords = c("longitude", "latitude"),
    crs = 4326,
  ...
  # 4. Extract coordinates in meters
  proj_coords <- st_coordinates(cuisine_projected) * FEET_TO_METERS
  
  # 5. Run DBSCAN with eps in meters
  db_result <- dbscan::dbscan(proj_coords, eps = eps, minPts = minPts)
  
  # 6. Add cluster assignments back to data
  cuisine_data <- cuisine_data %>%
    mutate(cluster = db_result$cluster)
  
  # Attach settings used (documenting that eps is in meters)
...
run_dbscan_by_borough <- function(data, cuisine,
                                  eps_values = c("Manhattan" = 350, 
                                                "Brooklyn/Queens" = 650, 
                                                "Bronx" = 600, 
                                                "Staten Island" = 700),
                                  min_floor = 4, 
                                  prop_factor = 0.015) {
  # NOTE: eps_values are now in METERS, not degrees!
  # Previous defaults (in degrees) have been converted:
  #   Manhattan:       0.004 deg ≈ 350 m
  #   Brooklyn/Queens: 0.0078 deg ≈ 650 m
  #   Bronx:           0.007 deg ≈ 600 m  
  #   Staten Island:   0.008 deg ≈ 700 m
  ...
  for (grp in unique(cuisine_data$boro_group)) {
    eps <- eps_values[[grp]]
    if (is.null(eps)) eps <- 500  # default fallback for "Other"
    
    sub_data <- cuisine_data %>% filter(boro_group == grp)
    
    if (nrow(sub_data) == 0) next
    
    # Convert to sf and project to NYC State Plane
    sub_sf <- st_as_sf(
      sub_data,
      coords = c("longitude", "latitude"),
      crs = 4326,
    ...
    sub_projected <- st_transform(sub_sf, crs = NYC_CRS)
    
    # Extract coordinates in meters
    proj_coords <- st_coordinates(sub_projected) * FEET_TO_METERS
    
    # Run DBSCAN with eps in meters
    db_result <- dbscan::dbscan(proj_coords, eps = eps, minPts = minPts)
  ...
  attr(final_data, "dbscan_params") <- list(
...
map_dbscan_clusters <- function(clustered_data) {
  ...
  # tmap v4 syntax:
  # - col → fill for fill color
  # - palette → fill.scale with tm_scale_categorical()
  # - title → fill.legend with tm_legend()
  # - alpha moved to fill_alpha
  tm_shape(clustered_sf) +
    tm_dots(
      fill = "cluster",
      fill.scale = tm_scale_categorical(values = colors),
      fill.legend = tm_legend(title = "Cluster ID"),
      size = 0.55,
      fill_alpha = 0.7
    )
...
map_cluster_polygons <- function(hulls) {
  ...
  # tmap v4 syntax:
  # - col → fill
  # - palette → fill.scale with tm_scale_continuous()
  # - alpha → fill_alpha
  # - title → fill.legend with tm_legend()
  tm_shape(hulls) +
    tm_polygons(
      ...
      fill.legend = tm_legend(title = "Restaurants in hotspot"),
      fill_alpha = 0.5
    )
...
# eps values are now in METERS (previously were in degrees)
# These values define the neighborhood radius for each borough group
eps_settings <- c(
  "Manhattan" = 350,        # tighter clusters in dense Manhattan
  "Brooklyn/Queens" = 650,  # slightly wider in outer boroughs
  "Bronx" = 600,
  "Staten Island" = 700     # widest for least dense borough
)
...
# Using meter-based eps values (function defaults are already in meters)
